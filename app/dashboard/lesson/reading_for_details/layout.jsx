...
export default function LessonLayout({ children }) {
  ...
  useEffect(() => {
    const initializeProgress = async () => {
      ...
      if (currentUser) {
        try {
          ...
          if (userDocSnap.exists()) {
            ...
            const currentStatus = userData.lessonStatus?.[5]?.status;
      ...
      const savedProgress = localStorage.getItem("reading_for_details_sub_progress");
      if (savedProgress) {
  ...
  useEffect(() => {
    const checkModuleCompletion = () => {
      const isCompleted = 
        isPartDone || 
        localStorage.getItem(`module_status_part_6_mod_${currentModuleOpen}`) === "completed";
...
function BottomBar({
  ...
  async function handleCompleteClick() {
    ...
    if (currentId < length) {
      ...
      localStorage.setItem("reading_for_details_sub_progress", nextProgress.toString());
      return;
    }
    ...
    try {
      ...
      if (userDocSnap.exists()) {
        ...
        if (currentStatusList.length > 0) {
          currentStatusList[5] = { ...currentStatusList[5], status: "done" };

          if (currentStatusList[6] && currentStatusList[6].status === "locked") {
            currentStatusList[6] = { ...currentStatusList[6], status: "progress" };
          }
