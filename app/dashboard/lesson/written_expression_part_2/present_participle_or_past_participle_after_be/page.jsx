...
export default function PresentParticipleOrPastParticipleAfterBe() {
  ...
  useEffect(() => {
    if (main_material && currentId) {
      ...
      // 🔒 STRICТ LOCKING SYSTEM
      if (isAlreadyCompleted || isNewlyCompleted) {
        setHasAnswered(true);

        // Amankan status ke localStorage jika baru diarahkan dari halaman kuis bawa parameter
        if (!isAlreadyCompleted && isNewlyCompleted) {
          localStorage.setItem(storageKey, "completed");
          window.dispatchEvent(new Event("practice-completed"));
        }
      } else {
        // 🌟 SAFETY LOCK: Paksa false jika tidak ada bukti penyelesaian di localStorage / URL
