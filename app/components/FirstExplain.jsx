  ...
  return (
    <div className={styles.container}>
      {sub_material.content[0].explain.map((item, i) => (
        <ColorBorderShadow key={i} borderColor={i == 0 ? "#C5502A" : "#2D7A5E"} className={styles.box}>
    ...
    <div className={styles.container}>
      {sub_material.content[0].explain.map((item, i) => (
        <div className={styles.box} key={i}>
          <ColorBorderShadow borderColor={i == 0 ? "#C5502A" : "#2D7A5E"} className={styles.box}>
            <HeaderVer1 item={item} i={i} />
            <div className={styles.divider}></div>
            {item.articles ? (
              <>
    ...
    <div className={styles.container}>
      {sub_material.content[0].explain.map((item, i) => (
        <ColorBorderShadow key={i} borderColor={i == 0 ? "#C5502A" : "#2D7A5E"} className={styles.box}>
          ...
          >
            {item.example_sentences.map((sentence) => (
              <p style={{fontWeight: 'normal', marginBottom: '5px'}} dangerouslySetInnerHTML={{ __html:  sentence}}  key={sentence}/>
            ))}
          </div>
          <p style={{color: 'white', backgroundColor: i == 1 ? "#8C8880" : "#C5502A", textAlign: 'center'}} className={styles.note}>{item.note}</p>
        </ColorBorderShadow>
      ))}
    </div>
  );
  ...
  return (
    <>
      <div className={styles.title}>
        <div>
          {item.subtitle ? <h5 style={{backgroundColor: i == 0 ? "#C5502A" : "#2D7A5E"}}>{item.subtitle}</h5> : <></>}
          <h3>{item.title}</h3>
        </div>
