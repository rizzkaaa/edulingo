...
export function calculateToeflScores({
  ...
  // Ambil nilai konversi berdasarkan jumlah jawaban benar + penyesuaian (Listening + 14, Structure + 12, Reading + 14)
  const rawListening = Math.min(50, Math.max(0, Math.round(listeningCorrect) + 14));
  const rawStructure = Math.min(40, Math.max(0, Math.round(structureCorrect) + 12));
  const rawReading = Math.min(50, Math.max(0, Math.round(readingCorrect) + 14));
