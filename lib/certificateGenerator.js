...
export async function ensureFontsLoaded() {
  ...
  const fontPromises = [
    document.fonts.load('46px "Hammersmith One"'),
    document.fonts.load('38px "Hammersmith One"'),
...
/**
 * Membagi nama peserta menjadi 2 baris jika terlalu panjang
 */
export function formatParticipantName(fullName = "") {
  const cleaned = (fullName || "PESERTA").trim().toUpperCase();
  
  // Jika panjang karakter <= 26, cukup 1 baris
  if (cleaned.length <= 26) {
    return [cleaned];
  }

  // Jika panjang, bagi menjadi 2 baris secara cerdas pada spasi terdekat dari tengah
...
export async function renderCertificateCanvas({
  ...
  return new Promise((resolve, reject) => {
    ...
    img.onload = () => {
      ...
      // 2. Format dan Gambar Nama Peserta
      const nameLines = formatParticipantName(fullName);
      ...
      if (nameLines.length === 1) {
        // 1 Baris Nama
        ctx.font = 'bold 56px "Hammersmith One", sans-serif';
        ctx.fillText(nameLines[0], 800, 540);
      } else {
        // 2 Baris Nama
        ctx.font = 'bold 48px "Hammersmith One", sans-serif';
        ctx.fillText(nameLines[0], 800, 518);
        ctx.fillText(nameLines[1], 800, 563);
      }
